...
app.get('/product/:id',ensureAuth, (req,res)=>{
    ...
    Product.findById(id,{ available:true})
    .populate('user', ' name email')
    .populate('category', 'description')
    .exec((err,product)=>{
        if(err){
...
app.delete('/deleteProduct/:id',ensureAuth, (req,res)=>{
    let id = req.params.id;
    let nAvailable = { available:false};

    Product.findByIdAndUpdate(id, nAvailable, {new:true}, (err,productDeleted)=>{
        if(err){
            res.status(400).json({
               ok: false,
               err
        ...
        if (!productDeleted){
            res.status(404).json({
                ok: false,
                err: {message:'El Articulo no ha podido ser borrado ya que no existe'}
            })
        }
        res.json({
            ok: true,
